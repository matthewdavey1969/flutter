...
class SliderThemeData with Diagnosticable {
  ...
  /// The size of the [BarSliderThumbShape] thumb.
  ///
...
class GappedSliderTrackShape extends SliderTrackShape with BaseSliderTrackShape {
  ...
  void paint(
    ...
    assert(sliderTheme.inactiveTrackColor != null);
    assert(sliderTheme.thumbShape != null);
    assert(sliderTheme.trackGapSize != null);
    assert(!sliderTheme.trackGapSize!.isNegative);
    // If the slider [SliderThemeData.trackHeight] is less than or equal to 0,
    // then it makes no difference whether the track is painted or not,
    // therefore the painting can be a no-op.
    if (sliderTheme.trackHeight == null || sliderTheme.trackHeight! <= 0) {
    ...
    final double thumbGapSize = sliderTheme.trackGapSize!;
    ...
    final RRect leftRRect = RRect.fromLTRBAndCorners(
      trackRect.left,
      trackRect.top,
      math.max(trackRect.left, thumbCenter.dx - thumbGapSize),
      trackRect.bottom,
      topLeft: trackCornerRadius,
      bottomLeft: trackCornerRadius,
